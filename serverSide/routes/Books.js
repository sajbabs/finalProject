  ...
  try {
    if (typeOfOpen == "a") {
      writeStream = fs.createWriteStream(
        path.join(__dirname, "../DataBase/books.txt"),
        { flags: "a" }
      );
    } else {
      writeStream = fs.createWriteStream(
  ...
  if (verify.role == "Admin") {
    ...
    if (!isItAlreadySaved(book)) {
      if (writeToFile(Data, "a")) {
        ...
        res.json({
          status: "success",
          message: "item faild to save, try again please",
        });
      }
    } else {
...
router.delete("/", function (req, res, next) {
  ...
  if (verify.role == "Admin") {
    ...
    res.json({
      ...
      role: req.headers.authorization.split(" ")[1],
    });
  }
});
    ...
    books.forEach(oldBook => {
      if (oldBook.id != "") {
        if (oldBook.id == book.id) {
          ...
          const old =
            oldBook.id + "," + oldBook.name + "," + oldBook.price + "\n";
